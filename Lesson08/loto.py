...
class Cards:
    ...
    def search(self, card_player, bag):
        for i, n in enumerate(card_player):
            if bag in n:
                card_player[i][n.index(bag)] = 'X'
                self.score += 1
                if self.score == 15:
...
def main():
    game = Cards(2)
    bag = Bag(90)
    player1 = Player('Карточка игрока')
    ...
    while True:
        ...
        if inp_user == 'y':
            if player1.search(game.card_user, bag):
                continue
            else:
                print('Game Over')
                sys.exit()
        if inp_user == 'n':
            if player1.search(game.card_user, bag):
                print('Game Over')
                sys.exit()
            elif player2.search(game.card_comp, bag):
                continue
